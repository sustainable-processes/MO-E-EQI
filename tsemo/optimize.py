    ...
    def fit_model(self, X, y):
        ...
        with warnings.catch_warnings():
            ...
            for i, v in enumerate(self.domain.output_variables):
                ...
                # from https://github.com/pytorch/botorch/issues/1847  (yizhe)
                # model = SingleTaskGP(X, y[:, [i]],
                #                      input_transform=Normalize(d=1),
                #                      outcome_transform=Standardize(m=1))

                # # Implement heteroskedastic noise levels
                # noise_level = 0.20
                # Y = self.all_experiments[v.name]
                # train_Yvar = ((Y * noise_level / np.std(Y)) ** 2).values.reshape(-1, 1).astype(np.float64)
