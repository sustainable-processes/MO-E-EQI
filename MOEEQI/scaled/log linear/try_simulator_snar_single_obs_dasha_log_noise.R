...
for (beta  in c(.6, .7, .8, .9)) {
  for (noise_slope in c(1.2, 0.8495)) {
    if (noise_slope == 1.2) {
      noise_level = -1.3
    } else{
      noise_level = -1.699
    ...
    for (n in 1:iter) {
      ...
      for (m in 1:initial_samples) {
        ...
        for (i in 1:MC_sample_size) {
          results <- ReactionSimulator_snar_new_log_linear(as.vector(unlist(input_data[m, ])), noise_slope, noise_level)
      ...
      noise_sd <-  data.frame(
        tau1 = (mean_STY / coeff) ^ noise_slope * 10 ^ noise_level / sty_sd,
        tau2 = mean_E.factor ^ noise_slope * 10 ^ noise_level / E.factor_sd
      )
      
      noise_sd <- tau_new_func(MC_sample_size, noise_sd, 1)
      # noise_sd[which(noise_sd[,2]==0),2] <- 1e-2
      ...
      tau_new <- data.frame(
        tau1 = ((
          predict.km(model_f1, newdata_scaled, type = "UK")$mean * sty_sd + sty_mean
        ) / coeff) ^ noise_slope * 10 ^ noise_level / sty_sd,
        tau2 =
          (
            predict.km(model_f2, newdata_scaled, type = "UK")$mean * E.factor_sd + E.factor_mean
          ) ^ noise_slope * 10 ^ noise_level / E.factor_sd
      )
      ...
      for (i in 1:Nsteps) {
        ...
        for (j in 1:MC_sample_size) {
          new_data <- ReactionSimulator_snar_new_log_linear(Scale(
            matrix(impr_x, 1, 4),
            a = rep(0, 4),
            b = rep(1, 4),
            l = ranges[, 1],
        ...
        tau_at_best_X <- tau_new_func(
          MC_sample_size,
          c((new_STY_mean / coeff) ^ noise_slope *
              10 ^ noise_level / sty_sd,
            (new_E.factor_mean) ^ noise_slope *
              10 ^ noise_level / E.factor_sd
          ),
          1
        )
      ...
      # save results csv
      res_folder <- paste('results_single_log_noise_',
                          noise_slope,
                          "_",
                          noise_level,
                          '_beta_',
