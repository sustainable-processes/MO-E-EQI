...
# PDFPath = paste(folder_path,"MOEEQI_",MOEEQI_set_up,"_scaled_inputs_outputs_HV_distance all betas.pdf",sep='')
# pdf(file=PDFPath, width=19, height=13)


# noise_level <- 0.2

for(noise_level in c(0.01,0.05,0.1,0.2)){
  over_HV_MOEEQI <- array(NA, dim=c(20,21,4))
  # HV_MOEEQI <- HV_qNEHVI <- HV_TSEMO <- log_diff_HV_MOEEQI <- log_diff_HV_qNEHVI <- log_diff_HV_TSEMO <- 
  
  for(name in alg_names){
  assign(paste("over_HV_",name,sep=""),NULL)
  }
  ...
  for(n in 1:20){
    ...
    for(name in alg_names){
      csv_name <- paste('../../code_python/results/linear/',name,'/TrainingSet1_linear_',noise_level_round,is_scaled,'/repeat_',n,'.csv', sep = "")
    ...
    for(beta in c(.6,.7,.8,.9)){
      ...
      MOEEQI_area_all_betas <- c(MOEEQI_area_all_betas, st_area(union_polygon))
    }
    over_HV_MOEEQI_n <- c(over_HV_MOEEQI_n,MOEEQI_area_all_betas)
    ...
    for(name in alg_names){
      ...
      over_HV_MOEEQI_n <- rbind(over_HV_MOEEQI_n, MOEEQI_area_all_betas)
    ...
    over_HV_MOEEQI[n,,] <- over_HV_MOEEQI_n
  ...
  over_HV_MOEEQI_mean <- apply(over_HV_MOEEQI,c(2,3),mean)
  over_HV_MOEEQI_min <- apply(over_HV_MOEEQI,c(2,3),quantile, probs=0.05)
  over_HV_MOEEQI_max <- apply(over_HV_MOEEQI,c(2,3),quantile, probs=0.95)
  ...
  over_HV_MOEEQI_plot <- NULL
  for(i in 1:4){
    beta <-  c(.6,.7,.8,.9)[i]
  over_HV_MOEEQI_plot <- rbind(over_HV_MOEEQI_plot,data.frame(
    mean=over_HV_MOEEQI_mean[,i], min=over_HV_MOEEQI_min[,i], max=over_HV_MOEEQI_max[,i], Method=paste('MOEEQI: \u03B2 = ',beta,sep=''), iter=1:21
  ))
  }
  ...
  for(name in alg_names){
    ...
    #     mean=get(paste("over_HV_",name,"_mean",sep="")), min=get(paste("over_HV_",name,"_min",sep="")), max=get(paste("over_HV_",name,"_max",sep="")), Method="LHS space filing", iter=1:21
    #   ))
    # }else{
    assign(paste("over_HV_",name,"_plot",sep=""), data.frame(
  ...
  # HV_plot <- rbind(over_HV_MOEEQI_plot,over_HV_qNEHVI_plot,over_HV_TSEMO_plot)
...
lemon::grid_arrange_shared_legend(plots[[1]],plots[[5]],
                                  plots[[2]],plots[[6]],
                                  plots[[3]],plots[[7]],
                                  plots[[4]],plots[[8]],
